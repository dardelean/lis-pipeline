...
if (Test-Path $ENV:WORKSPACE\env.properties){
    Remove-Item  $ENV:WORKSPACE\env.properties}
if (Test-Path $ENV:WORKSPACE\package.html){
    Remove-Item  $ENV:WORKSPACE\package.html}

Copy-Item "$UtilsDir\cookies_redhat.json" $env:WORKSPACE
Copy-Item "$UtilsDir\latest_versions.ps1" $env:WORKSPACE
pushd $env:WORKSPACE
...
if (Test-Path .\env.properties){
    $content=(Get-Content .\env.properties).Split("`r`n")
    $content[0]="CONTENT=$($content[0])"
    $content |  Out-File env.properties -Encoding ASCII }
    #$content |  Out-File kernel_version.txt -Encoding ASCII}
else{
    Write-Host "***********************"
    Write-Host "No new kernels were found !"
    Write-Host "***********************"
    #exit 1}
}

#for testing
$test_version = "3.10.0-830.el7"
$test_version | Out-File kernel_version.txt -Encoding ASCII
